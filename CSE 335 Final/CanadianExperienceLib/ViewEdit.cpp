...
void ViewEdit::OnLeftDoubleClick(wxMouseEvent &event)
{
    ...
    int currentFrame = (GetPicture()->GetAnimationTime() * GetPicture()->GetTimeline()->GetFrameRate());

    int machineCount = GetPicture()->GetMachines().size();
    for (int i = 0; i < machineCount; i++)
    {
        auto machine = GetPicture()->GetMachine(i);
        if (machine != nullptr && machine->HitTest(click))
        {
            machine->SetStartFrame(currentFrame);
            GetPicture()->UpdateObservers();
