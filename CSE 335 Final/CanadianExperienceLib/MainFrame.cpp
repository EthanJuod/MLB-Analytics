...
void MainFrame::Initialize()
{
    ...
    Bind(wxEVT_COMMAND_MENU_SELECTED, &MainFrame::OnChooseMachine, this, XRCID("ChooseMachine"));
...
/**
 * Handle for the event of choose machine
 * @param event
 */
void MainFrame::OnChooseMachine(wxCommandEvent& event)
{
    auto standardPaths = wxStandardPaths::Get();
    auto resourcesDir = standardPaths.GetResourcesDir().ToStdWstring();

    // Create a temporary machine just to show the dialog
    MachineSystemFactory factory(resourcesDir);
    auto tempMachine = factory.CreateMachineSystem();
    tempMachine->ChooseMachine(1);

    MachineDialog dialog(this, tempMachine);
    if (dialog.ShowModal() == wxID_OK)
    {
        // Get number that user inputed
        int machineNumber = tempMachine->GetMachineNumber();
        ...
        // Set position of specific machine
        if (machineNumber == 1)
        {
